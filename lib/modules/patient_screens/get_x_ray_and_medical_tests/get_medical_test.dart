...
class GetMedicalTestScreen extends StatefulWidget {
  const GetMedicalTestScreen({Key? key}) : super(key: key);

  @override
  GetMedicalTestScreenState createState() => GetMedicalTestScreenState();
}

class GetMedicalTestScreenState extends State<GetMedicalTestScreen> {
  @override
  void initState() {
    super.initState();
    GetXRayCubit.getCubit(context).getMedicalImages(patientID: patientId!);
  }
        ...
        builder: (BuildContext context, Object? state) {
          ...
          return Scaffold(
              ...
              body: ConditionalBuilder(
                  condition: getCubit.getMedicalImageModel?.data != null,
                  fallback: (context) => const Center(
                        child:
                            CircularProgressIndicator(color: Color(0XFF196A61)),
                      ),
                  builder: (context) => getCubit
                              .getMedicalImageModel?.data!.length ==
                          0
                      ? Center(
                          child: Column(
                            children: [
                              ...
                              defaultTextSegoe(
                                text: 'Not Defined Any Medical Tests',
                                fontSize: 30,
                      ...
                      : ListView.builder(
                          itemCount:
                              getCubit.getMedicalImageModel?.data!.length,
                          itemBuilder: (context, index) {
                            Data? currentData =
                                getCubit.getMedicalImageModel?.data![index];
